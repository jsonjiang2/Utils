    ...
    <script type="text/javascript">
        ...
        function createBuckleamount() {
            ...
            var $inputProfit = ""
                    ...
                    + "</label>"
                    + "<label class='control-label' style='padding-left:9px;'>"
                    + "<input class='form-control' name='buckleAmount' type='text' onblur='judgeSize("+xhSS+",this);' onkeyup='checkNum(this)' "
                    + "style='text-align:center;width:100px;line-height:18px;'>"
                    + "</label>"
                    + "<label class='control-label' style='padding-left:9px;'>"
        ...
        function queryUserName(idz,obj) {
            var action = "${pageContext.request.contextPath}/admin/deductionsRecord/findUserName";
        ...
        function judgeSize(idz,obj){
            ...
            if(parseFloat(buckleAmount) > avlBalance){
                alert("输入的扣除金额不能大于可用余额");
                $("#rs").html("");
                $("#totalf").html("");
                return false;
            }
            var action = "${pageContext.request.contextPath}/admin/deductionsRecord/total";
            var callback = function (data) {
                if(data.Msg != undefined){
                    alert(data.Msg);
                    return false;
                ...
                if(data.totalBuckleAmount != undefined){
                    $("#totalf").html(data.totalBuckleAmount);
                }
            }
        ...
        function insertFreeze() {
            ...
            var action = "${pageContext.request.contextPath}/admin/deductionsRecord/insertDRAmount";
            var callback = function (data) {
                ...
                if(data.status == "0"){
                    alert(data.Msg);
                    //添加按钮设置为可用
                    $("#KC").attr("disabled",false);
                    return false;
                }
                if(data.status == "1"){
                    alert(data.Msg);
                    window.location.href = "${pageContext.request.contextPath}/admin/deductionsRecord/lists";
                }
            }
            $.post(action, $("#insertFreezeForm").serialize(), callback);
        }
        ...
        <form id="insertFreezeForm" class="form-horizontal">
            ...
            <div id="manusl_freeze_Insert_top_div">
                <div style="font-size:15px;margin-left:450px;margin-top: 12px;">
                    ...
                    <label class="control-label" style="padding-left: 45px;">扣除余额</label>
                    <label class="control-label" style="padding-left: 100px;">扣款备注</label>
            ...
            <div id="manusl_freeze_Insert_div">
                <div class="form-group manuslfreeze" id="1">
                    <div style="margin-left:450px;">
                        ...
                        <label class="control-label" style="padding-left: 5px;">
                            <input class="form-control" name="buckleAmount" type="text" onblur="judgeSize(1,this);" onkeyup="checkNum(this)"
                                   style="text-align: center; width: 100px; line-height: 18px;">
                        </label>
                        <label class="control-label" style="padding-left: 5px;">
                            <input class="form-control" name="remark" type="text"
            ...
            <div class="form-group">
                ...
                <div class="col-md-2" style="margin-top:13px;font-size: 15px;">
                    ...
                    <label>，扣除总额：<span id="totalf" style="color:#ff8200"></span></label>
                </div>
            </div>
            ...
            <div class="form-group">
                <label class="col-md-4 control-label"></label>
                <div class="col-md-2">
                    <button id="KC" class="btn btn-primary" type="button" style="margin-left:-68px;"
                            onclick="insertFreeze()">确认扣除</button>
                </div>
            </div>
        </form>
    </div>
