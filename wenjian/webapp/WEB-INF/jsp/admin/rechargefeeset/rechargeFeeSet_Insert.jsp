...
<div class="container"  style="margin-top: 25px;">
    <form class="form-horizontal" role="form" id="insertForm">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-3">
                <select class="form-control" id="paycompanytype" name="paycompanytype" onchange="findBankName()">
...
<script type="text/javascript">
    ...
    function findBankName(){
        //充值设置ID
        var reid = $("#rechargesetid").val();
        //支付公司类型value值
        var obType = $("#paycompanytype").val();
        if (obType == "") {
            return false;
        }
        var action = "${pageContext.request.contextPath}/admin/rechargeFeeSet/findByPayCompanyType";
        var param = {
            "obtype": obType,
            "reid": reid
        };
        ...
        var callback = function (data) {
            if(data.Msg == "xssyyh"){
                html += '<option value="所有银行,ALL">所有银行</option>';
            }else if(data.Msg == "xsqtyh"){
                html += '<option value="其他银行,OTHER">其他银行</option>';
            }
            $.each(data.bankCodeNames, function (index, v) {
                html += '<option value="' + v.bankname + ',' + v.bankcode + '">' + v.bankname + '</option>';
            });
            $("#banknameid").html(html);
        };
