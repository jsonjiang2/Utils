...
$(function(){
	$("#insertModal").on("shown.bs.modal",function(){
		$("#resourcesGroup_insertForm").bootstrapValidator({
			icon: {
...
<div class="container"  style="margin-top: 25px;">
			<form class="form-horizontal" role="form"  id="resourcesGroup_insertForm">
			<%-- <c:if test="${empty preCommonMember}">
			    <label>暂无数据</label>
			</c:if> --%>
			<%-- <c:if test="${!empty preCommonMember}"> --%>
				<input type="hidden" id="id" name="id" value="${resourcesGroup.id}">
				<div class="form-group">
					<label class="col-sm-2 control-label laber_from" for="username">用户名</label>
					<div class="col-sm-3">
						<input type="text" name="username" id="username" value="${resourcesGroup.username}" class="form-control" />
					</div>
				</div>
				<div class="form-group">
					<label class="col-sm-2 control-label laber_from" for="ugroup">组别号</label>
					<div class="col-sm-3">
						<input type="text" name="ugroup" id="ugroup" value="${resourcesGroup.ugroup}" class="form-control" />
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-3">
						<input type="text" name="ugroupname" id="ugroupname" value="${resourcesGroup.ugroupname}" class="form-control" />
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-3">
						  <textarea rows="3" class="form-control" name="remark">${resourcesGroup.remark}</textarea>
					</div>
				</div>
