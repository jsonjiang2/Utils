...
<head>
    ...
    <script>
        ...
        //勾兑
        function blenDingByBatchno(batchno) {
            $("#gDuiBtn").attr("disabled",true);
            $.ajax({
                type: "post",
                url : "${pageContext.request.contextPath}/admin/userWithdrawalsBatch/blenDingByBatchno/",
                data : {"batchno":batchno},
                success:function (data) {
                    $("#gDuiBtn").attr("disabled",false);
                    if(data.status == "3"){
                        alert(data.Msg);
                ...
                <c:forEach items="${pagehelper.list }" var="entity" varStatus="tendsta">
                    <tr class="text-center">
                        ...
                        <td>
                            ...
                            <c:if test="${entity.status == 1 or entity.status == 2 or entity.status == 5}">
                                <button type="button" id="gDuiBtn" class="btn btn-default" onclick="blenDingByBatchno('${entity.batchno}')">勾兑</button>
                            </c:if>
                        </td>
                    </tr>
                </c:forEach>
